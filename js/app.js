...
stackWrappers.forEach((stackWrapper, wrapperIndex) => {
  const cards = stackWrapper.querySelectorAll(".stack-item");
  const stickySpace = stackWrapper.querySelector(".stack-offset");
  ...
  if (cards.length > 0) {
    function initCards() {
      ...
      //console.log(`initCards() wrapper ${wrapperIndex}`, cardHeight);
...
if (!innerDemoSlider) {
  const swiper = new Swiper(".mxd-demo-swiper", {
    ...
    loop: true,
    parallax: true,
    autoplay: { disableOnInteraction: false, enabled: false },
    grabCursor: true,
    speed: 1500,
    centeredSlides: true,
...
const initMarquee = () => {
  ...
  if (items.length) {
    items.forEach((itemBlock) => {
      const marquee = {
        el: itemBlock.querySelector(".marquee__toright"),
        width: 0,
        master: null,
      };

      marquee.width = marquee.el.offsetWidth;
      marquee.el.innerHTML += marquee.el.innerHTML;

      const dirFromRight = "+=50%";
      marquee.master = gsap
        .timeline()
        .add(marqueeRight(marquee.el, 50, dirFromRight), 0);

      const tween = gsap.to(marquee.master, {
        duration: 0.5,
        timeScale: 1.5,
        paused: true,
      });
      ...
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          marquee.master.timeScale(
            timeScaleClamp(Math.abs(self.getVelocity() / 600)),
          );
          tween.invalidate().restart();
        },
      });

      // Pausar y reanudar en hover
      itemBlock.addEventListener("mouseenter", () => marquee.master.pause());
      itemBlock.addEventListener("mouseleave", () => marquee.master.play());

      // Botones manuales
      const btnLeft = itemBlock.querySelector(".marquee-btn--left");
      const btnRight = itemBlock.querySelector(".marquee-btn--right");

      if (btnLeft && btnRight) {
        btnLeft.addEventListener("click", () => {
          marquee.master.pause();
          gsap.to(marquee.el, {
            x: "+=200",
            duration: 0.8,
            ease: "power2.out",
...
// --------------------------------------------- //
// Scroll to Top Button & WhatsApp Button Animations Start
// --------------------------------------------- //
const toTop = document.querySelector(".btn-to-top");
const whatsappBtn = document.querySelector(".btn-whatsapp-fixed");
...
if (whatsappBtn) {
  gsap.set(whatsappBtn, { opacity: 0 });
  gsap.to(whatsappBtn, {
    opacity: 1,
    autoAlpha: 1,
    scrollTrigger: {
...
// --------------------------------------------- //
// Scroll to Top Button & WhatsApp Button Animations End
// --------------------------------------------- //
